...
@section('container')
    ...
    @endif
    <form method="POST" action="{{ isset($teacher) ? route('teacher.update', $teacher->id) : route('teacher.store') }}" autocomplete="off" enctype="multipart/form-data">
        @csrf
        @if (isset($teacher))
            @method('put')
        @endif
            ...
            <div class="col-12 col-sm-4">
                <div class="form-group local-forms">
                    ...
                    <select class="form-control select" name="gender" id="gender" required>
                        ...
                        <option value="Laki-laki" {{ isset($teacher) && $teacher->gender === 'Laki-laki' ? 'selected' : '' }}>Male</option>
                        <option value="Perempuan" {{ isset($teacher) && $teacher->gender === 'Perempuan' ? 'selected' : '' }}>Female</option>
                    </select>
                </div>
            </div>
            <div class="col-12 col-sm-4">
